...
local function clipshot(arg)
    return function()
        ...
        os.execute('osascript -e \'set the clipboard to (read (POSIX file "/tmp/mpv-screenshot.jpeg") as JPEG picture)\'')
...
mp.add_key_binding('Alt+c',  'clipshot-video',  clipshot('video'))
mp.add_key_binding('Alt+V', 'clipshot-window', clipshot('window'))
